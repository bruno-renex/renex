...
function openKeyDB() {
  ...
  dbPromise = new Promise((resolve, reject) => {
    // VERSION 2: muss mit src/lib/idb.js übereinstimmen (Svelte-Frontend).
    // Wenn der Browser zuerst Svelte benutzt (DB → v2) und dann zur Legacy-Page
    // wechselt, wirft `open(... , 1)` einen VersionError. Beide Frontends nutzen
    // dieselbe Domain → dieselbe IDB → müssen identische Version anfragen.
    const req = indexedDB.open("renex-keys", 2);

    req.onupgradeneeded = () => {
      // Idempotent: Store wird angelegt falls er fehlt (deckt v0→v1, v1→v2,
      // sowie Legacy-DBs ab denen der Store nie erstellt wurde).
