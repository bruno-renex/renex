...
export async function fallbackBootstrap(me, peer, fetchInboxKeysFn, apiFetchFn) {
  ...
  try {
    // Inbox-Keys der Authority holen — mit Retry-Backoff weil
    // Cloudflare D1-Replikation nach /invite/join eine Sekunde dauern kann
    // (contacts-Eintrag muss propagieren, sonst verweigert /e2e/inbox/get).
    let inboxDevices = await fetchInboxKeysFn(peer);
    if (!Array.isArray(inboxDevices) || inboxDevices.length === 0) {
      for (const delay of [500, 1000, 2000]) {
        await new Promise(r => setTimeout(r, delay));
        const retry = await fetchInboxKeysFn(peer);
        if (Array.isArray(retry) && retry.length > 0) {
          console.log(`✅ Inbox-Keys nach ${delay}ms Retry verfügbar (${retry.length} devices)`);
          inboxDevices = retry;
          break;
        }
      }
    ...
    // AUSNAHME: wenn vorheriger Local-Only-Fallback gelaufen ist (Flag gesetzt),
    // kommt jetzt der echte Upload — Cache-ready blockiert NICHT, weil der
    // CMK noch nicht in KV ist.
    const wasLocalOnly = !!sessionStorage.getItem(`fallback_local_only:${sid}`);
    const existing = sessionCache.get(sid);
    if (existing?.ready && !wasLocalOnly) {
      console.log("⏭️ Fallback Bootstrap: echter CMK bereits im Cache — Fallback verworfen");
      return false;
    }
    if (wasLocalOnly) {
      console.log("🔁 Local-Only war aktiv — uploade jetzt CMK in KV (wasLocalOnly Flag)");
    }
    ...
    // Auch hier: wasLocalOnly überspringt die Blockade.
    if (sessionCache.get(sid)?.ready && !wasLocalOnly) {
      console.log("⏭️ Fallback Bootstrap: echter CMK nach KV-Store angekommen — Fallback verworfen");
      return false;
    }
    // Local-Only-Flag löschen — KV ist jetzt up-to-date
    try { sessionStorage.removeItem(`fallback_local_only:${sid}`); } catch {}
