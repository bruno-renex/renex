...
import { getDeviceId, idbGet, dmSessionId, deleteCMK, getCMKIfExists, wrapCMKForInboxDevices } from "./e2e.js";
import { isAuthority, ensureBootstrapped, receiveCMK, handleEpochRotate, rotateCMK, receiveCMKRotation } from "./sessionManager.js";

// Helper: lädt Inbox-Keys eines Users — wird von der cmk_req-Antwort genutzt
// um den eigenen lokalen CMK für anna's Devices zu wrappen.
async function _fetchInboxKeysForCmkUpload(user) {
  try {
    const r = await apiFetch(`/e2e/inbox/get?user=${encodeURIComponent(user)}`);
...
async function processControlMessage(m) {
  ...
  if (m.type === "cmk_req") {
    ...
    // Authority-Pfad: Standard-Bootstrap
    ...
    // NICHT-Authority: trotzdem antworten WENN ich einen lokalen CMK habe.
    // Hauptfall: Gast hatte fallbackBootstrap im Local-Only-Modus → CMK nur lokal.
    // Authority (Inviter) sendet cmk_req um den Gast-CMK in KV zu bekommen,
    // damit Authority alte Pre-Authority-Override-Messages decrypten kann.
    try {
      const localCmk = await getCMKIfExists(peer);
      if (!localCmk) return;
      const inboxDevices = await _fetchInboxKeysForCmkUpload(peer);
      if (!Array.isArray(inboxDevices) || inboxDevices.length === 0) return;
      const payloads = await wrapCMKForInboxDevices(inboxDevices.slice(-10), localCmk);
      if (payloads.length === 0) return;
      await apiFetch("/e2e/cmk/store", {
        method: "POST",
        body: JSON.stringify({ to: peer, payloads }),
      });
      console.log(`💾 Non-Authority CMK-Upload: lokalen CMK in KV für ${peer} gespeichert (${payloads.length} Devices)`);
    } catch (e) {
      console.warn("⚠️ Non-Authority cmk_req-Response fehlgeschlagen (non-fatal):", e?.message);
    }
    return;
  }
  ...
  if (m.type === "cmk_reset") {
    ...
    // VOR dem Löschen: lokalen CMK in KV hochladen (für Authority-Pre-Override-
    // Messages die mit dem alten CMK verschlüsselt waren). Best-Effort.
    try {
      const oldLocalCmk = await getCMKIfExists(peer);
      if (oldLocalCmk) {
        const inboxDevices = await _fetchInboxKeysForCmkUpload(peer);
        if (Array.isArray(inboxDevices) && inboxDevices.length > 0) {
          const payloads = await wrapCMKForInboxDevices(inboxDevices.slice(-10), oldLocalCmk);
          if (payloads.length > 0) {
            await apiFetch("/e2e/cmk/store", {
              method: "POST",
              body: JSON.stringify({ to: peer, payloads }),
            });
            console.log(`💾 Pre-Reset: alten CMK in KV hochgeladen (${payloads.length} Devices) — Authority kann Pre-Reset-Messages decrypten`);
          }
        }
      }
    } catch (e) {
      console.warn("⚠️ Pre-Reset CMK-Upload fehlgeschlagen (non-fatal):", e?.message);
    }
